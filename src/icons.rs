...
pub enum Icon {
    ...
    /// `nixos-update-building-NNN.png` where NNN is the percentage.
    Building(u8),
}
...
fn png_path(root: &Path, context: &str, slug: &str, size: u32) -> PathBuf {
    root.join(format!("hicolor/{size}x{size}/{context}"))
        .join(format!("nixos-update-{slug}.png"))
}
...
mod tests {
    ...
    fn decodes_to_argb32_big_endian() {
        let dir = std::env::temp_dir().join("nixos-update-manager-icon-test");
