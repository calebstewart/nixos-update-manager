...
pub struct Args {
    ...
    #[arg(long, env = "NIXOS_UPDATE_ICON_DIR")]
    ...
    #[arg(long, env = "NIXOS_UPDATE_TERMINAL")]
    ...
    #[arg(long, env = "NIXOS_UPDATE_EDITOR")]
    editor: Option<String>,

    /// Claude Code executable used by the troubleshooting session
    #[arg(long, env = "NIXOS_UPDATE_CLAUDE", default_value = "claude")]
    claude: String,

    /// The review dialog binary. Defaults to `nixos-update-review` sitting
    ...
    #[arg(long, env = "NIXOS_UPDATE_REVIEW_DIALOG")]
    ...
    #[arg(long, env = "NIXOS_UPDATE_CHECK_INTERVAL", value_parser = parse_span)]
    check_interval: Option<Duration>,

    /// Build an update as soon as a scheduled check finds one. Applying is
    /// never automatic.
    #[arg(long, env = "NIXOS_UPDATE_AUTO_BUILD")]
    auto_build: bool,
}
...
pub fn parse_span(text: &str) -> Result<Duration, String> {
    ...
    for c in text.trim().chars() {
        ...
        let unit = match c {
            ...
            other => {
                return Err(format!(
                    "unknown unit '{other}' in {text:?}; use s, m, h or d"
                ))
            }
        };
...
/// `$out/bin/.nixos-update-manager-wrapped`, i.e. the same directory -- unlike
/// the privileged apply helper, which has to climb out to libexec.
fn sibling_dialog() -> Option<PathBuf> {
    let exe = std::env::current_exe().ok()?;
    let candidate = exe.parent()?.join("nixos-update-review");
...
mod tests {
    ...
    fn terminal_args_may_start_with_a_hyphen() {
        ...
        let defaulted = Args::parse_from(["nixos-update-manager", "--flake", "/x"]);
        assert_eq!(defaulted.terminal_arg, vec!["-e".to_string()]);
    }

    /// There is no sensible default for the checkout, so the daemon refuses
    /// to start without one rather than guess at a path.
    #[test]
    fn flake_is_required() {
        assert!(Args::try_parse_from(["nixos-update-manager"]).is_err());
    }
}
