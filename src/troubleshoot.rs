...
fn prompt(report: &Path) -> String {
    format!(
        "The nixos-update-manager daemon just failed. Read the failure report at {} \
...
fn render(cfg: &Config, report: &ErrorReport) -> String {
    ...
    out.push_str("# nixos-update-manager failure\n\n");
    out.push_str(&format!(
        "Written {} by nixos-update-manager {}.\n\n",
...
mod tests {
    ...
    fn cfg() -> Config {
        Config {
            flake: PathBuf::from("/home/u/nixos"),
            host: "host".to_string(),
            user: "u".to_string(),
            branch: "nixos-update".to_string(),
            cache_dir: PathBuf::from("/home/u/.cache/nixos-update-manager"),
            icon_dir: None,
