...
const APP_ID: &str = "io.github.calebstewart.NixosUpdateManager.Review";
...
fn main() -> glib::ExitCode {
    ...
    let request = match read_request() {
        Ok(request) => request,
        Err(err) => {
            eprintln!("nixos-update-review: {err:#}");
    ...
    if request.version != PROTOCOL_VERSION {
        bail!(
            "review protocol mismatch: daemon sent v{}, this dialog speaks v{}. \
             ...
             `systemctl --user restart nixos-update-manager` and review again.",
            request.version,
            PROTOCOL_VERSION
        );
    }
...
fn build(app: &adw::Application, request: &ReviewRequest) {
    ...
    if !request.built {
        let text = match &request.plan {
            ...
            None => {
                "Build plan unavailable \u{2014} the check could not dry-run the build".to_string()
            }
        };
...
/// variables (`@accent_color` and the other libadwaita named colours the
/// desktop theme defines), and Pango markup only understands literal colours.
fn tally(counts: nixos_update_manager::Counts) -> gtk::Box {
...
fn package_row(package: &PackageChange) -> adw::ActionRow {
    ...
    let (label, class, subtitle) = if package.is_rebuild() {
        (
            "rebuilt",
            "kind-rebuilt",
            "same version, rebuilt".to_string(),
        )
    } else {
