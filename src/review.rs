...
use nixos_update_manager::{ReviewChoice, ReviewRequest};
...
mod tests {
    use super::*;
    use nixos_update_manager::{ApplyMode, Counts, Summary, PROTOCOL_VERSION};
    ...
    fn fake_dialog(name: &str, body: &str) -> (PathBuf, PathBuf) {
        ...
        let dir = std::env::temp_dir().join(format!("nixos-update-review-test-{name}"));
    ...
    fn a_missing_dialog_is_an_error_not_a_panic() {
        ...
        let reviewer = Reviewer::new(tx, PathBuf::from("/nonexistent/nixos-update-review"));
