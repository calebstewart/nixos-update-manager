...
use nixos_update_manager::BuildPlan;
...
fn program() -> String {
    std::env::var("NIXOS_UPDATE_NIX").unwrap_or_else(|_| "nix".to_string())
}
...
mod tests {
    ...
    fn fake_nix(name: &str, script: &str) -> PathBuf {
        let dir = std::env::temp_dir().join("nixos-update-manager-nix-test");
    ...
    fn streams_events_and_resolves_the_out_link() {
        ...
        let dir = std::env::temp_dir().join("nixos-update-manager-nix-test-ok");
        std::fs::create_dir_all(&dir).unwrap();
        ...
        std::env::set_var("NIXOS_UPDATE_NIX", &nix);
        ...
        std::env::remove_var("NIXOS_UPDATE_NIX");
    ...
    fn a_cancel_while_nix_runs_is_reported_as_such() {
        ...
        let mut tracker = Tracker::new();
        let link = std::env::temp_dir().join("nixos-update-manager-nix-test-cancel-link");
