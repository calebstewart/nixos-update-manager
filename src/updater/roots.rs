...
use nixos_update_manager::{Change, Counts, PackageChange, Scope};
...
mod tests {
    ...
    fn parses_what_the_apply_expression_yields() {
        let roots =
            parse(r#"[{"name":"nixvim","version":""},{"name":"firefox","version":"155.0"}]"#)
                .unwrap();
    ...
    fn classifies_changes_and_ignores_the_unchanged() {
        ...
        assert!(rows
            .iter()
            .all(|r| r.size.is_none() && r.scope == Scope::Home));
