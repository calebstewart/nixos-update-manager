...
use nixos_update_manager::{BuildPlan, PackageChange, Scope};
...
impl Worker {
    ...
    fn do_check(&self, previous: Option<&PendingUpdate>) -> Result<CheckOutcome> {
        ...
        let plan = nix::dry_run(&[&self.cfg.system_installable(), &self.cfg.home_installable()])?;
    ...
    fn do_build(&self, p: &PendingUpdate, quiet: bool) -> Result<BuildOutcome> {
            ...
            || git::hash_object(&worktree, "flake.lock")? != p.lock_hash
        {
            return Ok(BuildOutcome::Stale {
                message:
                    "The checked lock file is gone from the worktree; run Check for updates again."
                        .to_string(),
            });
        }
    ...
    /// `nixos-update-manager.service` (a new daemon binary at the least),
    ...
    /// activation died between those two phases: the shell, the polkit agent
    ...
    fn run_home_activation(&self, home_path: &Path) -> Result<()> {
        let output = Command::new("systemd-run")
            .args([
                ...
                "--collect",
                "--quiet",
                "--unit=nixos-update-manager-activate",
                "--description=nixos-update-manager home activation",
            ])
...
fn apply_helper() -> Result<PathBuf> {
    if let Ok(helper) = std::env::var("NIXOS_UPDATE_APPLY_HELPER") {
        return Ok(PathBuf::from(helper));
    }
    let exe = std::env::current_exe().context("resolving own executable path")?;
    exe.parent()
        .and_then(Path::parent)
        .map(|prefix| prefix.join("libexec/nixos-update-apply-system"))
        .filter(|helper| helper.exists())
        .ok_or_else(|| {
            anyhow!("nixos-update-apply-system helper not found; set NIXOS_UPDATE_APPLY_HELPER for dev runs")
        })
}
...
mod tests {
    ...
    fn a_check_comes_due_one_interval_after_it_was_armed() {
        ...
        assert_eq!(
            schedule.wakeup(now + Duration::from_secs(100)),
    ...
    fn a_due_timer_wakes_immediately_and_the_idle_sleep_is_bounded() {
        ...
        assert_eq!(
            schedule.wakeup(now + Duration::from_secs(5)),
            Duration::ZERO
        );
