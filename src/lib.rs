...
mod tests {
    ...
    fn choices_round_trip() {
        let mut choices: Vec<ReviewChoice> = ApplyMode::MENU_ORDER
            .iter()
            .map(|m| ReviewChoice::Apply(*m))
            .collect();
        choices.extend([ReviewChoice::Build, ReviewChoice::Dismiss]);
        for choice in choices {
            let encoded = serde_json::to_string(&choice).unwrap();
            assert_eq!(
                serde_json::from_str::<ReviewChoice>(&encoded).unwrap(),
                choice
            );
        }
    }
    ...
    fn shortens_revs_but_not_names() {
        ...
        assert_eq!(
            InputChange::short("github:numtide/flake-utils/v1.0.0"),
            "v1.0.0"
        );
